...
for ($i=1; $i -le $Passes; $i++) {
    ...
    $ds.Filter = '(|(&(objectCategory=person)(adminDisplayName=alpha))(&(objectCategory=person)(adminDisplayName=bravo))(&(objectCategory=person)(adminDisplayName=charlie)))'
    ...
    $ds.SearchScope = [System.DirectoryServices.SearchScope]::Subtree
    $ds.PageSize = $PageSize
    $ds.Filter = '(&(objectCategory=person)(|(adminDisplayName=alpha)(adminDisplayName=bravo)(adminDisplayName=charlie)))'
    [void]$ds.PropertiesToLoad.Add('distinguishedName')
    $res = $ds.FindAll()
    foreach ($item in $res) { 
        [void]$allUsers.Add($item.Properties['distinguishedName'][0]) 
